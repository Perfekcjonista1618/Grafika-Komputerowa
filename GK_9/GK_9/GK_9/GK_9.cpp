// GK_9.cpp : Defines the entry point for the console application.
//
...
typedef struct bpatch { // struktura dla wielomiana fragmentu Beziera 3 stopnia
	POINT_3D anchors[4][7]; // siatka 4x7 
...
void initBezier(void)
{
	for (int i = 0; i < 4; i++)
	{
		for (int p = 0; p < 5; p++)
		{
			mybezier.anchors[i][p] = makePoint(p, i, p + i / 2);
			mybezier.anchors[i][p].x = mybezier.anchors[i][p].x*sin(p*1.e-4	* (t++));
			mybezier.anchors[i][p].y = mybezier.anchors[i][p].y*sin(p*1.e-4 * (t++));
			mybezier.anchors[i][p].z = mybezier.anchors[i][p].z*sin(p*1.e-4 * (t++));
		}
	}
...
int InitGL(GLvoid)
{
	...
	GLboolean error = load_targa("sea_water_foam.tga", width_texture,
